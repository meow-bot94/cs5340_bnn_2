...
def transform_model(input, from_inst, to_inst, args={}, attrs={}, inplace=False):
    if inplace :
        output = input
    ...
    if isinstance(output, from_inst) :
        ...
        for name, module in output.named_children() :
            setattr(output, name, transform_model(module, from_inst, to_inst, copy.deepcopy(args), copy.deepcopy(attrs)))
