...
class PyroDensenet121Bnn(pyro.nn.PyroModule):

    def __init__(self, dataset: Dataset, device: str):
        super().__init__()
        self._net = self._create_pyro_resnet(dataset, device)
    ...
    def _create_resnet(self, dataset: Dataset, device: str):
        ...
        net.class_to_idx = dataset.class_to_idx
        ...
        net.softmax.requires_grad = True
        net.to(device)
        return net
    ...
    def _create_pyro_resnet(self, dataset: Dataset, device: str):
        net = self._create_resnet(dataset, device)
        self._convert_torch_to_pyro(net, device)
        return net
